    ...
    <div className="flex flex-col gap-10 w-96 border-l border-zinc-900 p-12 bg-zinc-950 z-10 shadow-[-10px_0_30px_rgba(0,0,0,0.3)]">
      ...
      <div className="flex flex-col gap-6 overflow-y-auto pr-4 -mr-4 scrollbar-hide">
        ...
        {groupedArmy.map(({ shadow, count }) => (
          <div key={`${shadow.name}-${shadow.rank}`} className="group flex flex-col gap-4 p-6 border border-zinc-900 bg-zinc-950 hover:bg-zinc-900/40 hover:border-zinc-700 transition-all duration-500 relative">
      ...
      <div className="mt-auto border-t border-zinc-900 pt-8 flex flex-col gap-2">
        ...
        <div className="text-2xl font-black italic text-zinc-300 tracking-tighter">
          {(army.reduce((acc, s) => acc + s.dps, 0)).toLocaleString()} <span className="text-[10px] text-zinc-600 ml-1">DPS</span>
        </div>
      </div>
    </div>
  );
